...
function fIdentificaCPF($cpf, $con){
$usu=leUsuarioPorCPF($cpf,$con);
if($usu){
    $ret="<h4> Identificando $cpf </h4>";
	$ret=$ret."<br><br><strong>Alcunha/Nome:</strong> ".$usu['apelido']." / ".$usu['nome'];
	$ret=$ret."<br><strong>Ativo? </strong>".$usu['ativo'];
	$ret=$ret."<br>Nível: ".$usu['nivel']." / ".$usu['nomenivel'];
	$ret=$ret."<br><strong>Fone: </strong>".$usu['fone'];
	$ret=$ret."<br>Email: ".$usu['email'];
	return $ret;
}else{
...
try {
    ...
    $_SESSION['msg']='ERRO: (lemedicopornome) ' . $e->getMessage(). ' '. $sql;
	return 0;
}	
}
...
/*	$sql="create temporary table pes001 (
		`ord` int(11) NOT NULL,
		...
		`ativo` varchar(1) COLLATE utf8_unicode_ci
	) ENGINE=MEMORY"; */
	$sqla="select id, cnpjcpf as cpf, nome, fone, email
		from pessoa
		where nome like '%$tex%'
		and id<>1
		order by nome";
	...
	try {
		...
		$_SESSION['msg']='ERRO: (fpesquisapessoa) ' . $e->getMessage(). ' '. $sql;
		//return false;
	}	
}
/************************** FPESQUISA  FIM  */
...
function fpesquisarequisicao ($nom,$dati,$datf,$con){
	...
	$sql="SELECT r.*, p.nome as pessoanome, p.fone,
		...
		on r.medico_id = m.id
		inner join pessoal.pessoa as p on p.id=r.pessoa_id
		where pessoanome like '%$nom%' and r.data between '$dati' and '$datf' order by id desc";
	}
	try {
		$rs=$con->query($sql);
		return $rs;
...
function flepessoapsicowebporid($id,$con){
try {
	$ssql="select pessoa.*, naturezadapessoa.descricao as naturezadapessoa
		from pessoa
		inner join naturezadapessoa on pessoa.naturezadapessoa_id=naturezadapessoa.id	
	...
	if($rs) {
		if($rs->rowCount()>0) {
			...
			$_SESSION['erro']='ERRO: Ao ler registro de usuario -> ' . $ssql;
			return false;
		}
	}else{
		$_SESSION['erro']='ERRO: Ao ler registro de usuario -> ' . $ssql;
    ...
    $_SESSION['erro']='ERROR: (conecta) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	}	
...
try {
	...
	if($rs) {
		if($rs->rowCount()>0) {
			...
			$_SESSION['erro']='ERRO: (leusuarioacl) Ao ler registro de usuario -> ' . $ssql;
			return false;
		}
	}else{
		$_SESSION['erro']='ERRO: (leusuarioacl) Ao ler registro de usuario -> ' . $ssql;
    ...
    $_SESSION['erro']='ERRO pdoexception: (leusuarioacl) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	}	

function lepessoafisicaporid($id,$con){
try {
	$ssql="select pessoafisica.*,pessoa.nome as pessoanome, pessoa.cnpjcpf as cpf,
		pessoa.denominacaocomum as apelido, pessoa.fone, pessoa.email
		from pessoafisica 
	...
	if($rs) {
		if($rs->rowCount()>0) {
			...
			$_SESSION['erro']='ERRO: Ao ler registro de pessoal -> ' . $ssql;
			return false;
		}
	}else{
		$_SESSION['erro']='ERRO: Ao ler tabela de Requisicoes -> ' . $ssql;
		return false;
	}	
} catch(PDOException $e) {
    $_SESSION['erro']='ERRO: (lepessoafisicaporid) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	}	
...
function fleestilo($estiloid,$conexaogr){
	$ssql = "select * from estilo where id = $estiloid";
	$leu=mysql_query($ssql,$conexaogr);
...
function flereport($repident,$conexaogr){
...
try {
    ...
    $_SESSION['msg']='ERROR: (fleenderecopessoa) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	
