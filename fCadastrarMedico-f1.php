...
if($lepessoa){
	...
	$_SESSION['msg']='Situação Inesperada ! (fCadastrarMedico-m1.php)';
	return;
}

$lemedico=lemedicoporpessoaid_fi($pid,$conefi);
if($lemedico){
	$idc=$lemedico['id'];
	$crm=$lemedico['crm'];
	$especialidade_id=$lemedico['especialidade_id'];
}
	
//$nome='';
$operacao=$_SESSION['operacao'];
$act=lcfirst($operacao).'Medico-f1.php?id='.$pid;
if($idc>0){
	$operacao='Atualizar';
	$act=$act.'&idc='.$idc;
	$rtab=lemedico_fi($idc,$conefi);
	if($rtab){
		//$nome=$rtab['nome'];
		$crm=$rtab['crm'];
...
$trace=ftrace('fCadastrarMedico-f1.php', $act);
?>
<script>
</script>
...
?>
		...
		<div class="row">
			<div class="form-group col-md-6">
				<a href="chameFormulario.php?op=consultar&obj=pessoa_medico&cpl=f1" > Cadastrar Novo Médico<a>
			</div>	
		</div>
    </div>    
</div>	
