	...
	try {
		...
		if($insert){
			...
			if($inclui){
				...
				$msg='Falha ao enviar mensagem:'.$query;
			}
		}else{ 
			$msg='Falha ao incluir hystory:'.$q1;
		}
	}catch (PDOException $e) {
		$msg="Erro PDOException (fincluimensagem.php): ".$e->getMessage()." ".$query;	
	}	
	return $msg;
}
...
$sql="select m.*, a.descricao as assunto, s.descricao as status
		...
		inner join msg_assunto as a on m.msg_assunto_id=a.id
		inner join msg_status as s on m.msg_status_id=s.id
		where (m.identificacao_destino='' or m.identificacao_destino like '$ident' or m.identificacao_origem like '$ident') and m.mensagem like '$tx' order by m.idbase, m.id ";
else		
$sql="select m.*, a.descricao as assunto, s.descricao as status
		from msg as m
		inner join msg_assunto as a on m.msg_assunto_id=a.id
...
try {
	...
	if($rs) {
		if($rs->rowCount()>0) {
			...
			$_SESSION['msg']='Falha: (leusuarioarh) Não encontrou registro de usuario -> ' . $ssql;
			return false;
		}
	}else{
		$_SESSION['msg']='Falha: (leusuarioarh) Ao ler registro de usuario -> ' . $ssql;
		return false;
	}	
} catch(PDOException $e) {
...
try {
	...
	if($rs) {
		if($rs->rowCount()>0) {
			...
			$_SESSION['msg']='Falha: Registro de Pessoa não encontrado -> ' . $ssql;
			return false;
		}
	}else{
		$_SESSION['msg']='ERRO: Ao ler tabela de Pessoa -> ' . $ssql;
		return false;
	}	
} catch(PDOException $e) {
    $_SESSION['msg']='ERRO Exception: (flepessoafisicaporid) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	}	
