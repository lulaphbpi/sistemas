...
function fleestagiarios($ordem, $con){
	$sql="SELECT pp.id AS pessoaid, 
			pp.nome, pu.ativo
			FROM rededesi_pessoal.pessoa AS pp
			INNER JOIN rededesi_psicoweb.usuario AS psiu ON psiu.pessoa_id=pp.id
			inner join rededesi_pessoal.usuario as pu on pu.pessoa_id=pp.id
			INNER JOIN rededesi_psicoweb.grupo AS pg ON pg.id=psiu.grupo_id
			WHERE pg.id=4 order by pp.nome";
} */
function leestagiarioporpessoaid($pid,$conpsi){
	$sql="SELECT e.*
			FROM rededesi_psicoweb.estagiario AS e
...
$sql="SELECT u.identificacao, p.id, p.nome, n.descricao AS nivel, u.ativo
	...
	INNER JOIN rededesi_psicoweb.usuario AS v ON v.pessoa_id=p.id
	INNER JOIN rededesi_psicoweb.niveldousuario AS n ON v.niveldousuario_id=n.id
...
$sql="SELECT t.*, p.nome, p.cnpjcpf AS cpf, p.denominacaocomum,
	...
	FROM rededesi_psicoweb.triagem AS t 
	...
	INNER JOIN rededesi_psicoweb.condicaodopaciente as c on c.id=t.condicaodopaciente_id
	inner join rededesi_psicoweb.requisicao as r on r.triagem_id=t.id
...
try {
	$sql = "select a.*,
			...
			from rededesi_arhweb.atendimento as a 
			inner join rededesi_arhweb.tipodeservico as t on a.tipodeservico_id = t.id
			inner join rededesi_arhweb.situacaoatendimento as s on a.situacaoatendimento_id = s.id
			inner join rededesi_arhweb.formadeatendimento as f on a.formadeatendimento_id = f.id
...
try {
	$sql = "select r.*, p.nome as pessoanome,
			...
			from rededesi_arhweb.requisicao as r 
			inner join rededesi_arhweb.tipodeservico as t on r.tipodeservico_id = t.id
			inner join rededesi_arhweb.situacaorequisicao as s on r.situacaorequisicao_id = s.id
			inner join rededesi_pessoal.pessoa as p on p.id=r.pessoa_id
...
try {	
	$ssql="select g.descricao as grupo, u.*, p.nome as pessoanome
			from rededesi_psicoweb.usuario as pu
			inner join rededesi_psicoweb.grupo as g on pu.grupo_id=g.id
...
try {
	$sql = "select r.*, p.nome as pessoanome,
			...
			from rededesi_psicoweb.requisicao as r
			inner join rededesi_pessoal.pessoa as p on r.pessoa_id=p.id
			inner join rededesi_pessoal.pessoafisica as f
			  on p.id=f.id
			inner join rededesi_psicoweb.situacaorequisicao as sr on r.situacaorequisicao_id=sr.id 
			inner join rededesi_psicoweb.tipodeatendimento as ta on ta.id=r.tipodeatendimento_id
			  where r.id=$rid
			order by r.id desc";
...
try {
	$ssql="select e.*, p.nome from rededesi_psicoweb.estagiario as e 
...
function fpesquisapessoapsi($tex,$conpes,$conpsi){
	...
	//  existirem em rededesi_psicoweb.pessoa e rededesi_psicoweb.usuario
...
function flehorariodeestagiario($id, $conpsi){
	$sql="SELECT psihe.id, pp.id as pessoaid,
			...
			FROM rededesi_psicoweb.horarioestagiario AS psihe
			INNER JOIN rededesi_pessoal.pessoa AS pp ON pp.id=psihe.pessoa_id
			INNER JOIN rededesi_psicoweb.diautil AS psidu ON psidu.id=psihe.diautil_id
			INNER JOIN rededesi_psicoweb.horario AS psih ON psih.id=psihe.horario_id where pp.id=$id"; 
...
function lehorariododia($pid, $dia, $hora, $conpsi){
	$sql="SELECT *
			FROM rededesi_psicoweb.horarioestagiario 
...
function lehorariodeestagiario($id, $conpsi){
	$sql="SELECT psihe.*, pp.id as pessoaid,
			...
			INNER JOIN rededesi_pessoal.pessoa AS pp ON pp.id=psihe.pessoa_id
			INNER JOIN rededesi_psicoweb.diautil AS psidu ON psidu.id=psihe.diautil_id
			INNER JOIN rededesi_psicoweb.horario AS psih ON psih.id=psihe.horario_id where psihe.id=$id"; 
...
$sql="
	...
	FROM rededesi_psicoweb.agenda AS a
	INNER JOIN rededesi_psicoweb.sala AS s ON a.sala_id=s.id
	INNER JOIN rededesi_psicoweb.horario AS h ON a.horario_id=h.id
	INNER JOIN rededesi_psicoweb.evento AS e ON a.evento_id=e.id
	INNER JOIN rededesi_pessoal.pessoa AS p ON a.responsavelpessoa_id=p.id
	INNER JOIN rededesi_psicoweb.requisicao AS r ON a.requisicao_id=r.id
	LEFT  JOIN rededesi_pessoal.pessoa AS p1 ON r.pessoa_id=p1.id
	INNER JOIN rededesi_psicoweb.ocorrenciaagendamento AS o ON a.ocorrenciaagendamento_id=o.id
...
function fmontahorariosemana($adata, $adia, $sala, $pessoa_id, $requisicao_id, $conpsi) {
...
$sql="SELECT he.*, p.denominacaocomum AS apelido,
	...
	FROM rededesi_psicoweb.horarioestagiario AS he
	INNER JOIN rededesi_psicoweb.diautil AS d ON he.diautil_id=d.id
	INNER JOIN rededesi_pessoal.pessoa AS p ON he.pessoa_id=p.id
	INNER JOIN rededesi_psicoweb.horario AS h ON he.horario_id=h.id";
try{	
	$rs=$conpsi->query($sql);
	if($rs) 
		return $rs;
...
try {
	$sql="SELECT r.*,s.descricao AS situacao,
		...
		FROM rededesi_psicoweb.requisicao AS r
		INNER JOIN rededesi_psicoweb.triagem AS t ON t.id=r.triagem_id
		INNER JOIN rededesi_psicoweb.situacaotriagem AS s
...
function fpesquisarequisicaoarhporpessoaid($pid,$dati,$datf,$con){
	$sql="SELECT r.*, p.nome as pessoanome, p.fone,
		t.descricao as tipo, s.descricao as situacao
		from rededesi_arhweb.requisicao as r 
		inner join rededesi_arhweb.tipodeservico as t
		on r.tipodeservico_id = t.id
		inner join rededesi_arhweb.situacaorequisicao as s
...
function fpesquisahistoricopsi($pid,$con){
	$sql="SELECT h.*, s.descricao as situacao
		from rededesi_psicoweb.historico as h
		inner join rededesi_psicoweb.requisicao as r on h.requisicao_id=r.id 
		inner join rededesi_psicoweb.situacaorequisicao as s
		on r.situacaorequisicao_id = s.id
...
function fpesquisapacientepsi ($nom,$dati,$datf,$con){
	// pesquisa rededesi_pessoal.pessoa presente em rededesi_psicoweb.requisicao e rededesi_psicoweb.pessoa
	if($nom==''){
	$sql="SELECT p.*, f.sexo, f.datanascimento,
		...
		from rededesi_pessoal.pessoa as p 
		inner join rededesi_pessoal.pessoafisica as f on f.id=p.id
		inner join rededesi_psicoweb.pessoa as pp on pp.pessoa_id=p.id
		inner join rededesi_psicoweb.naturezadapessoa as n on n.id=pp.naturezadapessoa_id
		inner join rededesi_psicoweb.requisicao as r on r.pessoa_id=pp.pessoa_id
		inner join rededesi_psicoweb.tipodeacompanhante as t on t.id=r.tipodeacompanhante_id
...
try {
	...
	FROM rededesi_pessoal.pessoa AS p
	INNER JOIN rededesi_arhweb.pessoa AS pp ON pp.pessoa_id=p.id
...
try {	
	$ssql = "select p.*, f.datanascimento, f.sexo, f.rg, f.expedidorrg_id,
			...
			from rededesi_arhweb.pessoa as pp
