...
$sql="SELECT pp.id AS pessoaid, 
		pp.nome
		FROM rededesi_pessoal.pessoa AS pp
		inner join rededesi_pessoal.pessoafisica as pf on pp.id=pf.id
		inner join rededesi_pessoal.institucional as i on i.pessoafisica_id=pf.id
...
function fleestagiarios($ordem, $con){
	$sql="SELECT pp.id AS pessoaid, 
			...
			inner join rededesi_pessoal.usuario as pu on pu.pessoa_id=pp.id
...
INNER JOIN rededesi_pessoal.pessoafisica AS f ON f.id=p.id
INNER JOIN rededesi_aclinica.examerequerido AS er ON er.requisicao_id=r.id
INNER JOIN rededesi_aclinica.exame AS e ON e.id=er.exame_id
INNER JOIN rededesi_aclinica.tipodeamostra AS ta ON ta.id=e.tipodeamostra_id
...
try {
	$sql = "select r.*, p.nome as pessoanome,
			...
			inner join rededesi_aclinica.statusrequisicao as st on r.statusrequisicao_id=st.id 
			  where r.pessoa_id=$pid
			order by r.id desc";
		//die($sql);	
	$rs= $con->query($sql);
...
try {
	$sql = "select a.*,
			...
			inner join rededesi_pessoal.usuario as u on u.id=a.usuario_id
			  where a.id='$id'";
	$rs= $conarh->query($sql);
	if($rs) {
		if($rs->rowCount()>0){
...
try {	
	$ssql="select g.descricao as grupo, u.*, p.nome as pessoanome
			...
			inner join rededesi_pessoal.usuario as u on pu.pessoa_id=u.pessoa_id
			inner join rededesi_pessoal.pessoa as p on u.pessoa_id=p.id
			order by p.nome";
...
try {
	$ssql="select e.*, p.nome from psicoweb.estagiario as e 
				inner join rededesi_pessoal.pessoa as p 
				on e.pessoa_id=p.id
				inner join rededesi_pessoal.pessoa as p1
...
function fpesquisapessoapsi($tex,$conpes,$conpsi){
	...
	//	e niveldousuario. Retorna todas as pessoas de rededesi_pessoal.pessoa independente de 
...
try {
	...
	INNER JOIN rededesi_pessoal.pessoafisica AS f ON f.id=c.consultapessoa_id
	INNER JOIN rededesi_pessoal.pessoa AS p ON p.id=f.id
	WHERE c.id=$id";
	$rs= $con->query($ssql); 
	if($rs) {
		if($rs->rowCount()>0) {
...
function fleestagiarios($conpsi){
	$sql="SELECT e.*, pp.id AS pessoaid, 
			pp.nome, pu.ativo
			from psicoweb.estagiario as e 
			inner join rededesi_pessoal.pessoa AS pp on e.pessoa_id=pp.id
			INNER JOIN psicoweb.usuario AS psiu ON psiu.pessoa_id=pp.id
			inner join rededesi_pessoal.usuario as pu on pu.pessoa_id=pp.id
			INNER JOIN psicoweb.grupo AS pg ON pg.id=psiu.grupo_id
			WHERE pg.grupo='Est' order by pp.nome";
...
function flehorariodeestagiario($id, $conpsi){
	$sql="SELECT psihe.id, pp.id as pessoaid,
			...
			INNER JOIN rededesi_pessoal.pessoa AS pp ON pp.id=psihe.pessoa_id
...
$sql="
	...
	INNER JOIN rededesi_pessoal.pessoa AS p ON a.responsavelpessoa_id=p.id
	INNER JOIN psicoweb.requisicao AS r ON a.requisicao_id=r.id
	LEFT  JOIN rededesi_pessoal.pessoa AS p1 ON r.pessoa_id=p1.id
...
$sql="SELECT he.*, p.denominacaocomum AS apelido,
	...
	INNER JOIN rededesi_pessoal.pessoa AS p ON he.pessoa_id=p.id
...
$sql="SELECT h.*, p.denominacaocomum AS apelido,
	...
	INNER JOIN rededesi_pessoal.pessoa AS p ON h.pessoa_id=p.id
...
function fpesquisapacientepsi ($nom,$dati,$datf,$con){
	// pesquisa rededesi_pessoal.pessoa presente em psicoweb.requisicao e psicoweb.pessoa
	if($nom==''){
