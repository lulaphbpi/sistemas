...
$con=false;
//if($database=='conup') die($database);
...
function fidentificalotacao($idlotacao,$convin){
    $ret="<h4> Identificando Lotação ".$idlotacao." </h4>";
	return $ret;
}

function fidentificatombo($idtombo,$convin){
    $ret="<h4> Identificando Tombo ".$idtombo." </h4>";
	return $ret;
}
...
if($rpes){
	...
	if($usunae){
		$txusu=$txusu."<br><strong>Grupo</strong> ".$usunae['descricaogrupo'];
	}	
	if($txusu=='')	{
...
function fleexamerequeridoexameid($id,$eid,$con){
try {
	$sql = "select er.*, e.sigla, e.descricao as exame,
			...
			inner join tipodeamostra as ta on er.tipodeamostra_id=ta.id
			...
			where r.id=$id and er.exame_id=$eid
			order by exame";
		//die($sql);	
	$rs= $con->query($sql);
	if($rs) {
    ...
    $_SESSION['msg']='ERRO: (fleexamerequeridoexameid) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
...
function fleexamerequeridoporpaciente($id,$con){
try {
	$sql = "select er.*, e.sigla, e.descricao as exame,
			e.observacao, r.data as datarequisicao,
			r.guia, m.nome as medico, m.crm,
			c.id as convenio_id, c.descricao as convenio,
			mt.descricao as metodo,
			ta.descricao as tipodeamostra,
			...
			inner join medico as m on r.medico_id=m.id
			inner join convenio as c on er.convenio_id=c.id
			inner join metodo as mt on mt.id=er.metodo_id
...
function fleexamerequeridoporcomponente($id,$con){
try {
	$sql = "select er.*, e.sigla, e.descricao as exame, est.medicamentos,
			est.observacoes, r.pessoa_id as pessoaid, r.data as datarequisicao,
			r.guia, m.nome as medico, m.crm, esp.descricao as especialidade,
			c.id as convenio_id, c.descricao as convenio,
			ce.descricao as componente,
			ce.id as componentedeexame_id,
			ce.unidade, ce.referencia,
			mt.descricao as metodo, ce.notas, ce.nvalor,
			ta.descricao as tipodeamostra,
			p.nome as pessoanome,
			p.cnpjcpf as cpf, p.denominacaocomum as apelido, p.fone, p.email,  f.datanascimento, 	
			f.sexo, f.rg, pe.nomemae, pe.cartaosus
			from examerequerido as er
			inner join exame as e on er.exame_id=e.id
			inner join tipodeamostra as ta on er.tipodeamostra_id=ta.id
			inner join requisicao as r on r.id=er.requisicao_id
			inner join medico as m on r.medico_id=m.id
			inner join especialidade as esp on m.especialidade_id=esp.id
			inner join convenio as c on er.convenio_id=c.id
			inner join componentedeexame as ce on ce.exame_id=e.id
			inner join metodo as mt on mt.id=er.metodo_id
			inner join pessoal.pessoa as p on r.pessoa_id=p.id
			inner join pessoal.pessoafisica as f on p.id=f.id			
			inner join pessoa as pe on p.id=pe.pessoa_id
			inner join estado as est on p.id=est.pessoa_id
			where r.id=$id
			order by exame, ce.ordem";
		//die($sql);	
	$rs= $con->query($sql);
	if($rs) {
		return $rs;
...
function fleestado($pid,$con){
try {
	$ssql="select * from estado where pessoa_id=$pid order by id desc";
	$rs= $con->query($ssql); 
	if($rs) {
		if($rs->rowCount()>0) {
			$reg=$rs->fetch(); 
			...
			//$_SESSION['msg']='Alerta: (fleestado) Id não encontrado $pid -> ' . $ssql;
			return false;
		}
	}else{
		//$_SESSION['msg']='ERRO: (fleestado) Ao ler registro de Pessoal -> ' . $ssql;
		return false;
	}	
} catch(PDOException $e) {
    $_SESSION['msg']='ERRO PDOException: (fleestado) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	
...
function flecodigodebarras3($id,$eid,$con){
	$sql = "select er.*, e.sigla, 
			...
			inner join convenio as c on er.convenio_id=c.id
			inner join componentedeexame as ce on ce.exame_id=e.id
			where er.requisicao_id=$id and er.exame_id=$eid and nvalor<>'0'
			order by exame, ce.ordem, ce.id"; // and e.id=$id
	//die($sql);
	try{
		$rs= $con->query($sql);
		if($rs) {
		...
		$_SESSION['msg']='ERRO: (flecodigodebarras3) ' . $e->getMessage(). ' '. $sql;
		return false;
	}	
}

function flecodigodebarras2($id,$eid,$con){
	$sql = "select er.*, e.sigla, 
			e.descricao as exame, 
			r.guia, m.nome as medico, m.crm,
			ce.descricao as componente,
			...
			inner join convenio as c on er.convenio_id=c.id
			inner join componentedeexame as ce on ce.exame_id=e.id
			where er.requisicao_id=$id and er.exame_id=$eid 
	...
	try{
		...
		$_SESSION['msg']='ERRO: (flecodigodebarras2) ' . $e->getMessage(). ' '. $sql;
		return false;
	}	
}
...
function flecodigodebarras($cod,$con){
	...
	$sql = "select er.*, e.sigla, 
			...
			inner join convenio as c on er.convenio_id=c.id
			inner join componentedeexame as ce on ce.exame_id=e.id
			where r.guia='$cod' and er.horacoletada<>''
			order by exame, ce.ordem, ce.id"; // and e.id=$id
...
function fleobservacoes($erid, $con){
try {
	$sql = "select o.*
			from observacoes as o
			where o.examerequerido_id=$erid
			order by tipo, id";
...
function fleitemcomponentedeexame($id,$con){
try {
	$sql = "select ce.*, i.valor1, i.valor2
			from componentedeexame as ce
			inner join itemexamerequerido as i on i.componentedeexame_id=ce.id
			where ce.exame_id=$id
...
function leitemexamerequeridoid($ider,$con){
try {
	$sql = "select ier.*
...
function leexamerequeridoexameid($id,$eid,$con){
try {
	$sql = "select er.*, e.sigla, 
			e.descricao as exame, 
			r.guia, m.nome as medico, m.crm,
			...
			inner join medico as m on r.medico_id=m.id
			inner join convenio as c on er.convenio_id=c.id
			where er.id=$id and er.exame_id=$eid
			order by er.id desc";
	$rs= $con->query($sql);
	if($rs) {
		if($rs->rowCount()>0){
    ...
    $_SESSION['msg']='ERRO: (leexamerequeridoexameid) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
...
try {
	$sql = "select e.*, t.descricao as material, s.descricao as setor
			from exame as e
			inner join tipodeamostra as t on e.tipodeamostra_id=t.id
			inner join setor as s on s.id=e.setor_id
			order by setor, e.descricao";
...
function fleexameativorequerido($rid, $con){
try {
	$sql = "select e.*, t.descricao as material, 
			s.descricao as setor, m.descricao as metodo
			from exame as e
			inner join examerequerido as er on er.exame_id=e.id
			inner join tipodeamostra as t on er.tipodeamostra_id=t.id
			inner join setor as s on s.id=e.setor_id
			inner join metodo as m on m.id=er.metodo_id
			where er.requisicao_id=$rid and ativo='S'
			order by setor, e.descricao";
...
try {
	$sql = "select escala.*
			from escala
			where data='$dt' and (quantidade < limite) 
			order by data, id"; //die($sql);
	$rs= $con->query($sql);
	if($rs) {
		if($rs->rowCount()>0){
			$rec=$rs->fetch();
...
function lefisioterapeutaporpessoaid_fi($pid,$con){
try {
	$sql = "select f.*, e.descricao as especialidade
			from fisioterapeuta as f
			inner join especialidade as e on f.especialidade_id=e.id
			where f.pessoa_id=$pid
			order by f.id desc";
    ...
    $_SESSION['msg']='ERRO: (lefisioterapeutaporpessoaid_fi) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function lemedicoporpessoaid_fi($pid,$con){
try {
	$sql = "select m.*, e.descricao as especialidade
			from medico as m
			inner join especialidade as e on m.especialidade_id=e.id
			where m.pessoa_id=$pid
			order by m.id desc";
    ...
    $_SESSION['msg']='ERRO: (lemedicoporpessoaid_fi) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
...
function lefisioterapeuta_fi($id,$con){
try {
	$sql = "select f.*, e.descricao as especialidade
			from fisioterapeuta as f
			inner join especialidade as e on f.especialidade_id=e.id
    ...
    $_SESSION['msg']='ERRO: (lefisioterapeuta_fi) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function lemedico_fi($id,$con){
try {
	$sql = "select m.*, e.descricao as especialidade
			from medico as m
			inner join especialidade as e on m.especialidade_id=e.id
    ...
    $_SESSION['msg']='ERRO: (lemedico_fi) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
...
try {
    ...
    $_SESSION['msg']='ERRO: (lemedico) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
...
function fconcluiavaliacaofisica($agendaid){
$conefi=conexao('efisio');
$conefi->beginTransaction();
try {
	$ssql = "update agenda set statusagenda_id=2 where id=$agendaid and statusagenda_id<2";
	$rs = $conefi -> query($ssql);
	if($rs) {
		$conefi->commit();
		return true;
	...
	$conefi->rollback();
    $_SESSION['msg']='ERRO: (fconcluiavaliacaofisica) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	
}
...
function letratamento_fi($tid){
	$con=conexao('efisio');

	$sql="select * from tratamento where id=$tid";
	try {
		$rs=$con->query($sql);
		if($rs){
			if($rs->rowCount()>0){
		...
		$_SESSION['msg']='ERROR: (letratamento-fi' . $e->getMessage(). ' '. $sql;
		return false;
	}	
}	
...
function flequestionarioaplicado($pid){
$conefi=conexao('efisio');
$ssql = "SELECT DISTINCT qa.*, pp.denominacaocomum AS operador, 
		a.data as dataagenda, qq.titulo, ta.descricao as tipodeagenda,
		sa.descricao as statusagenda,
		a.statusagenda_id, ss.descricao AS situacao,
		s.descricao AS servico, sp.ativo
...
function leagenda_fi($agendaid){
$conefi=conexao('efisio');
$ssql = "select a.*, s.descricao as servico
		from efisio.agenda as a
		inner join servicopessoa as ss on a.servicopessoa_id=ss.id
		inner join servico as s on ss.servico_id=s.id
		where a.id=$agendaid";
try {
	$rs= $conefi->query($ssql);
	if($rs) {
    ...
    $_SESSION['msg']='ERRO: (leagenda_fi) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	
}
...
function fpesquisaagendaoperador_fi($tx, $tipoagendaid, $id, $con){
try {	
	$ssql="select a.*, pu.denominacaocomum as operador, p.nome as cliente, 
			...
			sp.id as sid, s.descricao as servico, ta.descricao as tipodeagenda
		from efisio.agenda as a
		...
		where a.usuario_id=$id AND p.nome LIKE '%$tx%'
		  and a.tipodeagenda_id=$tipoagendaid
    ...
    $_SESSION['msg']='ERRO PDOException: (fpesquisaagendaoperador_fi) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}	

function leagendaoperador($agendaid, $tipoagendaid, $con){
try {	
	$ssql="select a.*, pu.denominacaocomum as operador, p.nome as cliente, 
			ta.descricao as tipodeagenda, ss.descricao as statusservico,
...
function fleagendaoperador($usu, $con){
try {	
	$ssql="select a.*, pu.denominacaocomum as operador, p.nome as cliente, 
		...
		inner join efisio.servico as s on sp.servico_id=s.id
		where a.usuario_id=$usu and a.statusagenda_id=1
		order by a.data desc, a.horainicial"; 
...
try {	
	$ssql="select t.* 
		...
		inner join agenda as a on a.id=t.agenda_id
		inner join servicopessoa as sp on a.servicopessoa_id=sp.id
		where sp.id=$spid
    ...
    $_SESSION['msg']='ERRO PDOException: (flehistoricopaciente) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}	
...
try {
	$sql = "select sp.*, sv.descricao as servico,
...
try {
    ...
    $_SESSION['msg']='ERRO: (lepessoanae) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
...
function leunidade($uni, $con){
try {
	$sql = "select u.*,
		u.descricao as unidade, m1.descricao as marcaunidade, 
		t.descricao as tipounidade
		from unidade AS u
        inner join marca as m1 on u.id_marca=m1.id		
		inner join tipo as t on u.id_tipo=t.id 
		where u.id=$uni"; //$t=ftrace('leunidade', $sql);
	$rs= $con->query($sql);
	if($rs) {
		if($rs->rowCount()>0){
			$rec=$rs->fetch(); 
    ...
    $_SESSION['msg']='ERRO PDOException: (leunidade) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function fleunidade($con){
try {
	$sql = "select u.*,
		u.descricao as unidade, m1.descricao as marcaunidade, 
		t.descricao as tipounidade
    ...
    $_SESSION['msg']='ERRO PDOException: (fleunidade) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
...
function fleup($con){
try {
	$sql = "SELECT up.*, d.tombo, d.descricao AS dispositivo, m.descricao AS marcadispositivo,
		s.descricao AS statusdispositivo, 
		u.descricao AS unidade, m1.descricao AS marcaunidade, 
		t.descricao AS tipounidade, e.descricao AS especificacao,
		s1.descricao AS statusup 
		FROM up 
		INNER JOIN dispositivo AS d ON up.id_dispositivo=d.id 
		INNER JOIN marca AS m ON d.id_marca=m.id
		INNER JOIN STATUS AS s ON d.id_status=s.id
		INNER JOIN unidade AS u ON up.id_unidade=u.id
        INNER JOIN marca AS m1 ON u.id_marca=m1.id		
        INNER JOIN tipo AS t ON u.id_tipo=t.id		
		INNER JOIN especificacao AS e ON u.id_especificacao=e.id
		INNER JOIN STATUS AS s1 ON u.id_status=s1.id
		ORDER BY tombo, u.id"; 
	$rs= $con->query($sql);
	if($rs) {
		if($rs->rowCount()>0){
    ...
    $_SESSION['msg']='ERRO PDOException: (fleup) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function fleunidadedetombo($idt,$con){
try {
	$sql = "SELECT up.*, d.tombo, d.descricao AS dispositivo, m.descricao AS marcadispositivo,
		s.descricao AS statusdispositivo, 
		u.descricao AS unidade, m1.descricao AS marcaunidade,
		t.descricao as tipounidade,	
		s1.descricao AS statusunidade, e.descricao AS especificacao
		FROM up 
		INNER JOIN dispositivo AS d ON up.id_dispositivo=d.id 
		INNER JOIN marca AS m ON d.id_marca=m.id
		INNER JOIN STATUS AS s ON d.id_status=s.id
		...
		INNER JOIN especificacao AS e ON u.id_especificacao=e.id
		INNER JOIN STATUS AS s1 ON u.id_status=s1.id
		where d.tombo='$idt' order by id"; //$t=ftrace('fleunidadedetombo',$sql);
	$rs= $con->query($sql);
	if($rs) {
		if($rs->rowCount()>0){
			return $rs;
    ...
    $_SESSION['msg']='ERRO PDOException: (fleunidadedetombo) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function letombo($idt,$con){
try {
	$sql = "select d.*, m.descricao as marcadispositivo,
		s.descricao as statusdispositivo 
		from dispositivo as d 
		inner join marca as m on d.id_marca=m.id
		inner join status as s on d.id_status=s.id
		where tombo='$idt'"; 
	$t=ftrace("letombo",$sql);	
	$rs= $con->query($sql);
	if($rs) {
		if($rs->rowCount()>0){
			$rec=$rs->fetch(); 
    ...
    $_SESSION['msg']='ERRO PDOException: (letombo) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
...
function letomboidup($id,$con){
$sql="select up.*, d.tombo
		from up 
		inner join dispositivo as d on up.id_dispositivo=d.id
	    where up.id=$id";
try {
    ...
    $_SESSION['msg']='ERRO PDOException: (fledispositivo) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
	
function fledispositivo($con){
$sql="select d.*, m.descricao as marcadispositivo, 
		s.descricao as statusdispositivo
		from dispositivo as d 
		inner join marca as m on d.id_marca=m.id
...
function fpesquisadispositivo($tx,$con){
$tx=trim($tx);	
if(!$tx=='')
$sql="SELECT d.*
	FROM dispositivo as d 
	where (d.tombo like '$tx%' or d.descricao like '$tx%')
	ORDER BY d.tombo, d.descricao";
else		
$sql="SELECT d.*
	FROM dispositivo as d 
	ORDER BY d.tombo, d.descricao";
try {
    ...
    $_SESSION['msg']='ERRO PDOException: (fpesquisaDispositivo) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function fpesquisaUP($tx,$con){
$tx=trim($tx);	
if(!$tx=='')
$sql="SELECT up.*, d.tombo, d.descricao AS dispositivo, m.descricao AS marcadispositivo,
		s.descricao AS statusdispositivo, 
		u.descricao AS unidade, u.id_tipo, u.id_especificacao, u.id_status,
		m1.descricao AS marcaunidade, 
		t.descricao AS tipounidade, s1.descricao AS statusunidade,
		e.descricao as especificacaounidade
	FROM up 
		INNER JOIN dispositivo AS d ON up.id_dispositivo=d.id 
		INNER JOIN marca AS m ON d.id_marca=m.id
		INNER JOIN STATUS AS s ON d.id_status=s.id
		...
		INNER JOIN especificacao as e on u.id_especificacao = e.id
		INNER JOIN STATUS AS s1 ON u.id_status=s1.id
	where (d.tombo like '$tx%' or d.descricao like '$tx%' or u.descricao like '$tx%')
	ORDER BY d.tombo, u.descricao";
else		
$sql="SELECT up.*, d.tombo, d.descricao AS dispositivo, m.descricao AS marcadispositivo,
		s.descricao AS statusdispositivo, 
		u.descricao AS unidade, u.id_tipo, u.id_especificacao, u.id_status,
...
try {
    ...
    $_SESSION['msg']='ERRO PDOException: (fpesquisaUP) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
...
function fpesquisamnemonico($tex,$con){
$tex=trim($tex);
if($tex=='')
$sql="SELECT a.*
		from mnemonico as a
...
$sql="SELECT a.*
		from mnemonico as a
		where a.sigla like '%$tex%' or 
		a.descricao like '%$tex%' or
		a.latim like '%$tex%' order by a.id";
try {
//	$trace=ftrace('fpesquisamnemonico',$sql);
	$rs= $con->query($sql);
	return $rs; 
} catch(PDOException $e) {
    $_SESSION['msg']='ERRO: (fpesquisamnemonico) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function fpesquisametodo($tex,$con){
$tex=trim($tex);
if($tex=='')
$sql="SELECT a.*
		from metodo as a
...
$sql="SELECT a.*
		from metodo as a
		where a.descricao like '%$tex%' order by a.descricao";
try {
//	$trace=ftrace('fpesquisametodo',$sql);
	$rs= $con->query($sql);
	return $rs; 
} catch(PDOException $e) {
    $_SESSION['msg']='ERRO: (fpesquisametodo) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function fpesquisaamostra($tex,$con){
$tex=trim($tex);
if($tex=='')
$sql="SELECT a.*
		from tipodeamostra as a
		order by a.descricao";
else	
$sql="SELECT a.*
		from tipodeamostra as a
		where a.descricao like '%$tex%' order by a.descricao";
try {
//	$trace=ftrace('fpesquisaamostra',$sql);
	$rs= $con->query($sql);
	return $rs; 
} catch(PDOException $e) {
    $_SESSION['msg']='ERRO: (fpesquisaamostra) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
...
$tex=trim($tex);
if($tex=='')
$sql="SELECT e.*, t.descricao as tipodeamostra, s.descricao as setor
		from exame as e
		inner join tipodeamostra as t on e.tipodeamostra_id=t.id
		inner join setor as s on s.id=e.setor_id
...
$sql="SELECT e.*, t.descricao as tipodeamostra, s.descricao as setor
		...
		inner join tipodeamostra as t on e.tipodeamostra_id=t.id
		inner join setor as s on s.id=e.setor_id
		where e.ativo='S' order by e.descricao";
else	
$sql="SELECT e.*, t.descricao as tipodeamostra, s.descricao as setor
		from exame as e
		inner join tipodeamostra as t on e.tipodeamostra_id=t.id
...
function fIdentificaAmostra($id,$con){
	return $id;
}
...
function fpesquisadiautil($tex, $con){
	$tex=trim($tex);
	if($tex==''){
		$sql = "select * from diautil order by dia desc";
	}else{
		$d=formataData($tex);
		$sql = "select * from diautil 
			where dia = 'd'
			order by id desc";
	}
	try {
		...
		$_SESSION['msg']='ERRO PDOException: (fpesquisadiautil) ' . $e->getMessage(). ' '. $sql;
		return false;
	}	
}

function fleconsultanae($id,$con){
try {
	$ssql="SELECT c.id AS consulta_id, c.cpfresponsavel, 
	c.tipodeacompanhante_id, c.convenio_id, 
	c.medico_id, c.dataregistro, c.dataconsulta, 
	c.horario, c.confirmado, c.realizado, 
	f.*, p.cnpjcpf AS cpf, m.nome as medico, m.crm,
	cv.descricao as convenio,
	p.denominacaocomum AS apelido,
	p.nome, p.fone, p.email
	FROM consnae.consulta AS c
	inner join medico as m on m.id=c.medico_id
	inner join convenio as cv on cv.id=c.convenio_id
	INNER JOIN pessoal.pessoafisica AS f ON f.id=c.consultapessoa_id
	INNER JOIN pessoal.pessoa AS p ON p.id=f.id
	WHERE c.consultapessoa_id=$id order by consulta_id desc";
	$rs= $con->query($ssql); 
	if($rs) {
		return $rs;
	}else{
    ...
    $_SESSION['msg']='ERRO PDOException: (fleconsultanae) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	}	
...
function fpesquisaagendatratamento_fi($tex){
	//$conpes=conexao("pessoal");
	$conefi=conexao("efisio");
	
	$sql="SELECT sp.*, es.descricao AS servico,
	pp.nome, pp.fone, pf.datanascimento, pf.sexo,
	ss.descricao as status, a.id as agendaid
	FROM efisio.servicopessoa AS sp 
	inner join efisio.agenda as a on sp.id=a.servicopessoa_id
	inner join efisio.statusservico as ss on sp.statusservico_id=ss.id
	...
	WHERE sp.ativo='N' AND (sp.statusservico_id=3 or sp.statusservico_id=6) and a.tipodeagenda_id=2
		AND pp.nome LIKE '%$tex%'
	ORDER BY pp.nome, servico";
	try {
		$rs= $conefi->query($sql); 
		if($rs) 
			return $rs; 
	} catch(PDOException $e) {
		$_SESSION['msg']='ERRO Exception: (fpesquisaagendatratamento_fi) ' . $e->getMessage(). ' '. $sql;
	}	
}

function fpesquisaservicopessoaemtratamento_fi($tex){
	//$conpes=conexao("pessoal");
	$conefi=conexao("efisio");
	...
	INNER JOIN pessoal.pessoafisica AS pf ON pp.id=pf.id
	INNER JOIN efisio.servico AS es ON sp.servico_id=es.id
	WHERE sp.ativo='N' AND (sp.statusservico_id=2 or sp.statusservico_id=3) and a.statusagenda_id=2
		AND pp.nome LIKE '%$tex%'
	ORDER BY pp.nome, servico";
	try {
		$rs= $conefi->query($sql); 
		...
		$_SESSION['msg']='ERRO Exception: (fpesquisaservicopessoaemtratamento_fi) ' . $e->getMessage(). ' '. $sql;
	}	
}

function fpesquisaservicopessoaavaliada_fi($tex){
	//$conpes=conexao("pessoal");
	$conefi=conexao("efisio");
	...
	INNER JOIN pessoal.pessoafisica AS pf ON pp.id=pf.id
	INNER JOIN efisio.servico AS es ON sp.servico_id=es.id
	WHERE sp.ativo='N' AND sp.statusservico_id=1 and a.statusagenda_id=2
		AND pp.nome LIKE '%$tex%'
	ORDER BY pp.nome, servico";
	try {
		$rs= $conefi->query($sql); 
		...
		$_SESSION['msg']='ERRO Exception: (fpesquisaservicopessoaavaliada_fi) ' . $e->getMessage(). ' '. $sql;
	}	
}

function fpesquisaservicopessoacadastrada_fi($tex){
	//$conpes=conexao("pessoal");
	$conefi=conexao("efisio");
	...
	INNER JOIN pessoal.pessoafisica AS pf ON pp.id=pf.id
	INNER JOIN efisio.servico AS es ON sp.servico_id=es.id
	WHERE sp.ativo='N' AND sp.statusservico_id<3 
		AND pp.nome LIKE '%$tex%'
	ORDER BY pp.nome, servico";
	try {
		$rs= $conefi->query($sql); 
...
function fpesquisapessoa_fi($tex){
	$conpes=conexao("pessoal");
	$conefi=conexao("efisio");
	// pessoal
	$sqla="select pessoa.id, cnpjcpf as cpf, 
		denominacaocomum as apelido, nome, fone, email, sexo, datanascimento
		from pessoal.pessoa inner join pessoafisica on pessoa.id=pessoafisica.id
		where pessoa.situacao_id<3 and nome like '%$tex%'
		and pessoa.id<>1
		order by nome";
	// efisio
	$sqla1="select n.descricao as naturezadapessoa, p.nomemae, c.id, c.descricao as cor 
		...
		where p.pessoa_id=";
	$sqla2="select u.ativo, g.grupo, g.descricao as nomegrupo
...
function fpesquisapessoanae($tex){
	...
	// consnae
	$sqla1="select n.descricao as naturezadapessoa, p.nomemae, c.id, c.descricao as cor 
		from consnae.pessoa as p
		inner join consnae.naturezadapessoa as n on p.naturezadapessoa_id=n.id
		inner join consnae.cor as c on p.cor_id=c.id
		where p.pessoa_id=";
	$sqla2="select u.ativo, g.grupo, g.descricao as nomegrupo
	...
	$cont=0;	
	// consnae
	$sqla4="select * from consnae.consulta as c where c.dataconsulta>=NOW() and c.consultapessoa_id=";
	$sqltruncate="truncate table pes001";
	try {
		...
		if($rsqla) {
			if($rsqla->rowCount()>0) {
				foreach ($rsqla->fetchAll() as $recsqla) {
					...
					$sqli="insert into consnae.pes001 values (
						$ord,$pessoaid,'$natureza','$cpf','$nome','$sexo','$dtn','$fone','$cartaosus','$email','$nivel','$ident','$status','$ativo')";
					$sql=$sqli;
					$i=$connae->query($sqli);
				}	
		...
		$sqlt="select * from consnae.pes001 order by ord";
		$rsqlt=$connae->query($sqlt);
		if($rsqlt) {
			return $rsqlt;
		}
	} catch(PDOException $e) {
		$_SESSION['msg']='ERRO Exception: (fpesquisapessoanae) ' . $e->getMessage(). ' '. $sql;
		//return false;
	}	
}
...
function leagendamentoporcpf($cpf, $con){
	$sql="select * from pessoaagenda where cnpjcpf='$cpf'";
	try {
		$rs=$con->query($sql);
		if($rs){
			if($rs->rowCount()>0){
		...
		$_SESSION['msg']='ERROR: (leagendamentoporcpf ' . $e->getMessage(). ' '. $sql;
		return false;
	}	
}	
...
function fpesquisapessoa($nom,$con){
	if($nom == NULL or empty($nom))
	$sql="SELECT p.*, f.datanascimento, f.sexo, f.rg, f.expedidorrg_id, f.formacaoprofissional_id
		from pessoa as p
		inner join pessoafisica as f on f.id=p.id 
		inner join expedidorrg as e on f.expedidorrg_id=e.id
		inner join formacaoprofissional as fp on f.formacaoprofissional_id=fp.id
		order by p.nome";
	else	
	$sql="SELECT p.*, f.datanascimento, f.sexo, f.rg, f.expedidorrg_id, f.formacaoprofissional_id
		from pessoa as p
		...
		inner join expedidorrg as e on f.expedidorrg_id=e.id
		inner join formacaoprofissional as fp on f.formacaoprofissional_id=fp.id
		where p.nome like '%$nom%' order by p.nome";
	try {
		$rs=$con->query($sql);
		return $rs;
	}catch (PDOException $e) {
		$_SESSION['msg']='ERRO PDOException: (fpesquisapessoa) ' . $e->getMessage(). ' '. $sql;
		return false;
	}	
}
...
function fpesquisasala($txt, $con) {
	...
	$trace=ftrace('fpesquisasala',$sql);
	try {
		$rs=$con->query($sql);
		return $rs;
	}catch (PDOException $e) {
...
function fpesquisatipodeequipamento($txt, $con) {
	$txt = trim($txt);
	if($txt=='') {
		$sql = "select t.*
				from tipodeequipamento as t
				order by t.id";
	}else{
		$sql = "select t.*
				from tipodeequipamento as t
	...
	try {
		...
		$_SESSION['msg']='ERRO PDOException: (fpesquisatipodeequipamento) ' . $e->getMessage(). ' '. $sql;
		return false;
	}	
}

function fpesquisalotacao($txt, $con) {
	$txt = trim($txt);
	if($txt=='') {
		$sql = "select l.*, s.codigo
				from lotacao as l
				inner join sala as s on s.id=l.id_sala
				order by s.codigo, l.siape";
	}else{
		$sql = "select l.*, s.codigo
				from lotacao as l
				inner join sala as s on s.id=l.id_sala
				where l.siape=$txt or l.nome like '%$txt%'
				order by s.codigo, l.siape";
	}	
	$trace=ftrace('fpesquisalotacao',$sql);
	try {
		$rs=$con->query($sql);
		return $rs;
	}catch (PDOException $e) {
		$_SESSION['msg']='ERRO PDOException: (fpesquisalotacao) ' . $e->getMessage(). ' '. $sql;
		return false;
	}	
}

function fpesquisatombo($txt, $con) {
	$txt = trim($txt);
	if($txt=='') {
		$sql = "select t.*, e.descricao as tipodeequipamento
				from tombo as t
				inner join tipodeequipamento as e on e.id=t.id_tipodeequipamento
				order by e.descricao, t.tombo";
	}else{
		$sql = "select t.*, e.descricao as tipodeequipamento
				from tombo as t
				inner join tipodeequipamento as e on e.id=t.id_tipodeequipamento
				where t.tombo=$txt or e.descricao like '%$txt%'
				order by e.descricao, t.tombo";
	}	
	$trace=ftrace('fpesquisatombo',$sql);
	try {
		$rs=$con->query($sql);
		return $rs;
	}catch (PDOException $e) {
		$_SESSION['msg']='ERRO PDOException: (fpesquisatombo) ' . $e->getMessage(). ' '. $sql;
		return false;
	}	
}
...
function identificacaoexiste($ident, $con){
	$sql = "SELECT * from usuario where identificacao = '$ident'"; 
	$rs= $con->query($sql);
	if($rs) {
		if($rs->rowCount()>0) {
			$reg=$rs->fetch(); 
		...
		$_SESSION['msg']='Falha (identificacaoexiste): Não leu identificaca -> ' . $sql;
		return false;
	}	
}	
...
try {
	...
	if($rs) {
		...
		$_SESSION['msg']='ERRO: (leusuariosis) Ao ler registro de usuario -> ' . $ssql;
		return false;
	}	
} catch(PDOException $e) {
    $_SESSION['msg']='ERRO pdoexception: (leusuariosis) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	}	

//Substituir por leusuariosis, onde convier
...
function fpesquisausuario($tx, $con) {
$tex=trim($tx);
if($tex=='')
$sql="SELECT u.*, p.nome, n.descricao as nivel, g.descricao as grupo
		from pessoal.usuario as u
		inner join pessoal.pessoa as p on u.pessoa_id=p.id
		left join ibibea.usuario as us on u.pessoa_id=us.pessoa_id 
		inner join ibibea.niveldousuario as n on us.niveldousuario_id=n.id
		inner join ibibea.grupo as g on us.grupo_id=g.id		
		order by p.nome";
else	
$sql="SELECT u.*, p.nome, n.descricao as nivel, g.descricao as grupo
		from pessoal.usuario as u
		...
		inner join ibibea.niveldousuario as n on us.niveldousuario_id=n.id
		inner join ibibea.grupo as g on us.grupo_id=g.id		
		where p.nome like '%$tex%' order by p.nome";
try {
	$rs= $con->query($sql);
	return $rs; 
} catch(PDOException $e) {
    $_SESSION['msg']='ERRO: (fpesquisausuario) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function lealunoporid_ea($id, $con) {
$sql="SELECT p.*, a.id as alunoid, a.identificacao
		from pessoal.pessoa as p
		left join ibibea.aluno as a on a.pessoa_id=p.id
		where p.id = $id order by p.nome";
try {
	$rs= $con->query($sql);
	if($rs)
		if($rs->rowCount()>0){
    ...
    $_SESSION['msg']='ERRO: (lealunoporid_ea) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function leprofessorporid_ea($id, $con) {
$sql="SELECT p.*, a.id as professorid, a.identificacao
		from pessoal.pessoa as p
		left join ibibea.professor as a on a.pessoa_id=p.id
		where p.id = $id order by p.nome";
try {
	$rs= $con->query($sql);
	if($rs)
    ...
    $_SESSION['msg']='ERRO: (leprofessorporid_ea) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function fpesquisaaluno($tx, $con) {
$tex=trim($tx);
if($tex=='')
$sql="SELECT p.*, a.id as alunoid, a.identificacao
		from pessoal.pessoa as p
...
try {
    ...
    $_SESSION['msg']='ERRO: (fpesquisaaluno) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}

function fpesquisaprofessor($tx, $con) {
$tex=trim($tx);
if($tex=='')
$sql="SELECT p.*, a.id as professorid, a.identificacao
		from pessoal.pessoa as p
...
try {
    ...
    $_SESSION['msg']='ERRO: (fpesquisaprofessor) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}
...
function validaregistro_ea($r, $con){
	$ret='';
	// Comprovar existencia de registro
	$sql1 = "select * from registro where id=$r";
	$l1=$con->query($sql1);
	if($l1){
		if($l1->rowCount()>0){
			// Verificar condição do registro
			$rl=$l1->fetch();
			$cat1=$r1['cativo'];
			$bai1=$r1['baixado'];
			$sub1=$r1['substituido'];
			...
			$emp1=$r1['emprestado'];
			$res1=$r1['reservado'];
			$sst=$cat1+$bai1+$sub1+$res1+$emp1+$res1;
			if($sst>0){
				$ret='Registro de Obra Indisponível';
			}	
		}else{
			$ret='Registro de Obra Não Encontrado';
		}	
	}else{	
		$ret='Registro de Obra Não Encontrado';
	}	
	...
	if(empty($ret)){
		$sql2 = "select * from emprestimo where registro_id=$r and (statusemprestimo_id=1 or statusemprestimo_id=4)";
		$l2=$con->query($sql2);
		if($l2)
			if($l2->rowCount()>0)
				$ret='Registro de Obra Não se encontra Disponível para Empréstimo';
	}	
	return $ret;
}	

function flistaemprestimo_ea($pid,$con){
	$sql = "select e.*, o.tipoobra_codigo, o.artigo, o.titulo, s.descricao as status
			from ibibea.emprestimo as e 
			inner join pessoal.pessoa as p on e.pessoa_id=p.id
			inner join ibibea.registro as r on r.id=e.registro_id
			inner join ibibea.obra as o on r.obra_id=o.id
			inner join ibibea.statusemprestimo as s on e.statusemprestimo_id=s.id
			where p.id=$pid
			order by e.dataemprestimo desc, e.horaemprestimo desc";
try {
	$rs= $con->query($sql);
	return $rs; 
} catch(PDOException $e) {
    $_SESSION['msg']='ERRO: (flistaemprestimo_ea) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}	
...
$sql = "select q.*, t.nalternativas as permiteopcoes,
		...
		from questao as q
		inner join tipoquestao as t on q.id_tipoquestao=t.id
		where id_questionario=$idq and q.id>$idc order by ordem, q.id";
try {
	$rs= $con->query($sql);
	return $rs; 
} catch(PDOException $e) {
...
function letipodeagendaid_fi($id,$con){
	$rec=false;
	$sql="select s.*
	...
	try {
		...
		$_SESSION['msg']='ERRO: (letipodeagenda_fi) ' . $e->getMessage(). ' '. $sql;
		return false;
	}
	//if($rec) die($sql); else die('nao leu');
	return $rec;
...
function lestatusservicoid_fi($id,$con){
	$rec=false;
	$sql="select s.*
	...
	try {
		...
		$_SESSION['msg']='ERRO: (lestatusservico_fi) ' . $e->getMessage(). ' '. $sql;
		return false;
	}
	//if($rec) die($sql); else die('nao leu');
	return $rec;
...
function leregistro($id,$con) {
	$ssql = "select * from registro where id=$id";
	$leu=$con->query($ssql);
	if($leu)
		if($leu->rowCount()>0){
			$rleu=$leu->fetch();
...
function fleregistrosdaobra($id,$con) {
	$nleu=false;
	$registros="";
	$ssql = "select * from registro where obra_id=$id order by id";
	$leu=$con->query($ssql);
	if($leu)
		if($leu->rowCount()>0)
			return $leu;
...
function fautoresdaobra($id,$con) {
	$autores="";
	$ssql = "select autordescricao from obraautor where obra_id=$id";
	$leu=$con->query($ssql);
	if($leu) {
		if($leu->rowCount()>0){
		foreach($leu->fetchAll() as $rec){
			if($autores==""){
				$autores=$rec['autordescricao'];
			}else{
				$autores=$autores." /// ".$rec['autordescricao'];
			}
		}
		}
	}
...
function fleautoresdaobra($id,$con) {
	$assuntos="";
	$ssql = "select * from obraautor where obra_id=$id order by id";
	$leu=$con->query($ssql);
	if($leu)
		if($leu->rowCount()>0)
			return $leu;
...
function fleassuntosdaobra($id,$con) {
	$assuntos="";
	$ssql = "select * from obraassunto where obra_id=$id order by id";
	$leu=$con->query($ssql);
	if($leu)
		if($leu->rowCount()>0)
			return $leu;
...
function fassuntosdaobra($id,$con) {
	$assuntos="";
	$ssql = "select assuntodescricao from obraassunto where obra_id=$id";
	$leu=$con->query($ssql);
	if($leu) {
		if($leu->rowCount()>0){
		foreach($leu->fetchAll() as $rec){
			if($assuntos==""){
				$assuntos=$rec['assuntodescricao'];
			}else{
				$assuntos=$assuntos." /// ".$rec['assuntodescricao'];
			}
		}
		}
	}
...
function fregistrosdaobra($id,$con) {
	$registros="";
	$ssql = "select * from registro where obra_id=$id order by id";
	$leu=$con->query($ssql);
	if($leu) {
		if($leu->rowCount()>0){
		foreach($leu->fetchAll() as $rec){
			$iditem=fnumero($rec['id'],5);	
			$ano = $rec['ano'];
			$data = pegaDatadeDataHora($rec['dataregistro']);
			$tomo = $rec['tomo'];
			$volume = $rec['volume'];
			$exemplar = $rec['nroexemplar'];
			$origem = $rec['origem_codigo'];
			$edicao = $rec['edicao'];
			$status="";
			if($rec['cativo']){$status="Cativo";}
			if($rec['reservado']){$status="Reservado";}
			if($rec['emprestado']){$status=$status." - Emprestado";}
			if($rec['baixado']){$status=$status." - Baixado";}
			if($rec['substituido']){$status=$status." - SubstituÃƒÂ­do";}
			if($rec['restauracao']){$status=$status." - RestauraÃƒÂ§ÃƒÂ£o";}				
			$registros=$registros.$iditem." ".$ano." ".$data." ".$exemplar." ".$edicao." ".$status."\n"; 
		}
		}
	}
	return $registros;
...
function fatualizaobra_ea($id,$tipoobra_codigo,$artigo,$tituloobra,$codcdd,$cutter,
	$ehdopiaui,$edicao,$local_id,$editora_id,$anopublicacao,$numeropaginas,
	$serie,$cduasc,$artigocdd,$sumario, $usu,$con) {
	$dataalteracao=date("y-m-d h:m:s");
	if($id>0)
		$query="UPDATE obra set tipoobra_codigo='$tipoobra_codigo',artigo='$artigo',
			titulo='$tituloobra',codcdd='$codcdd',cutter='$cutter',ehdopiaui='$ehdopiaui',
			edicao='$edicao',local_id='$local_id',editora_id='$editora_id',
			anopublicacao='$anopublicacao',numeropaginas='$numeropaginas',
			serie='$serie',cduasc='$cduasc',artigocdd='$artigocdd',
			sumario='$sumario' where id=$id";
	else{		
		$id=fproximoid("obra",$con);
		$query = "insert into obra values ('$id','$tipoobra_codigo','$artigo','$tituloobra','$codcdd',
			'$cutter','$ehdopiaui','$edicao','$local_id','$editora_id','$anopublicacao','$numeropaginas',
			'$serie','$cduasc','$artigocdd','$sumario','$dataalteracao')";
	}
	$q=addslashes($query);
	$ok=false;
	$con->beginTransaction();
	try{
		$hys=incluihystory('fatualizaobra-ea', $q, $usu, $con);
		if($hys){
		...
		if($ok) {
			...
			$_SESSION['msg']=' Falha. Operação Não Realizada!';
		}
} catch(PDOException $e) {
	$con->rollback();
    $_SESSION['msg']='ERRO: (fincluiobra_ea) ' . $e->getMessage(). ' '. $query;
}
if($ok)
	return $id;
else
...
function leemprestimoid_ea($id,$con){
	$rec=false;
	$sql="select e.*, o.titulo, s.descricao as status
		FROM emprestimo as e
		inner join registro as r on e.registro_id=r.id
		inner join obra as o on r.obra_id=o.id
		inner join statusemprestimo as s on e.statusemprestimo_id=s.id
		where e.id = $id"; 
	try {
		$exec= $con->query($sql);
		if($exec){
		...
		$_SESSION['msg']='ERRO: (leemprestimoid_ea) ' . $e->getMessage(). ' '. $sql;
		return false;
	}
	//if($rec) die($sql); else die('nao leu');
	return $rec;
...
function leregistroid_ea($id,$con){
	$rec=false;
	$sql="select r.*
		FROM registro as r
		where r.id = $id"; 
	try {
		...
		$_SESSION['msg']='ERRO: (leregistroid_ea) ' . $e->getMessage(). ' '. $sql;
		return false;
	}
	//if($rec) die($sql); else die('nao leu');
	return $rec;
...
function leobraid_ea($id,$con){
	$rec=false;
	$sql="select obra.*, tipoobra.descricao as tipoobradescricao, 
		       local.descricao as localdescricao, editora.descricao as editoradescricao
		FROM obra
		inner join tipoobra on tipoobra.codigo=obra.tipoobra_codigo
		inner join local on local.id=obra.local_id
	...
	try {
		...
		$_SESSION['msg']='ERRO: (leobraid_ea) ' . $e->getMessage(). ' '. $sql;
		return false;
	}
	//if($rec) die($sql); else die('nao leu');
	return $rec;
...
function fpesquisaobra_ea($txt,$con){
	$txt=trim($txt);
	if($txt==''){
		$sql="select obra.*, tipoobra.descricao as tipoobradescricao, 
		       local.descricao as localdescricao, editora.descricao as editoradescricao
...
function fidentifica_h01_ea(){
	$m1="<h1>Ajuda</h1>";
	$m1=$m1."<p>1. Digite parte do título e clique na lupa para consultar uma obra. Considere acentuação.</p>";
	return $m1;	
}
