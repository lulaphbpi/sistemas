...
$act=lcfirst($operacao).'Statusservico-f1.php';
$id=0;
...
if($id>0){
	$operacao='Atualizar'; 
	$act=$act.'?id='.$id;
	$rtab=lestatusservicoid_fi($id,$conefe);
	if($rtab){
	...
	if(isset($_GET['del']) && empty($_GET['del']) == false){
		$del=$_GET['del'];
		if($del=='del'){
			$operacao='Excluir - Cuidado: esteja certo de não existir status de serviço relacionado a outra estrutura';
...
<div class="areatrabalho">
    <div class="formularioEntrada">
		<form action="<?php echo($act);?>" method="post" id="iformulario" class="formulario">
			...
			<p><a href='chameFormulario.php?op=consultar&obj=statusservico&cpl=f1&idx=<?PHP echo($tx);?>'>Retorna Consulta</a>	</p>		
