...
$rotina='fConsultarMedico-f1.php';
...
if(isset($_POST['textopesquisa']) && empty($_POST['textopesquisa']) == false){
	...
	if(strlen($tx)>1)
		$tx=trim($tx);
	$reclst=fpesquisamedico_fi($tx,$conefi);
	if(!$reclst)
		die($_SESSION['msg']);
...
<div class="areatrabalho">
	...
	<div class="areareduzida">
        <form action="" method="post" id="iformulario" class="formulario">
			...
			<p>Digite o texto a pesquisar no nome (Arquivo de Médicos): </p><br>
			<div class="input0">
			<input class="inputpesquisa" type='text' name='textopesquisa' id='ipesquisa' value=" " />
			<button type="button" id="ibotao" class="lupa"></button>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="chameFormulario.php?op=consultar&obj=pessoa_medico&cpl=f1" > Cadastrar Médico<a>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="rmedico01.php" target="_new" > Relação Geral de Médicos</a>
			</div>
			<br><br>
		</form>
	<br>
...
medico&&cpl=f1&id=<?PHP echo($pid);?>&idc=$medicoid'><?PHP echo($medicoid);?></td>
		<td><?PHP echo($nome);?></td>
		<td><?PHP echo($crm);?></td>
		<td><?PHP echo($especialidade);?></td>
		<td><a href='chameFormulario.php?op=cadastrar&obj=
medico&cpl=f1&id=<?PHP echo($pid);?>&del=<?php echo('del');?>'><?PHP echo('Exc');?></a></td>
    </tr>
<?php
	}
}
?>
	...
	<tr>
		...
		?>
		<td><a href='chameFormulario.php?op=consultar&obj=medico&cpl=f1&id=<?php echo($ord); ?>'>Próximos</a></td>
		<?php	
		}
		?>
	</tr>	
